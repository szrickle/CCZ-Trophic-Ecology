...
ccz_ltu <- ccz_ltu %>% mutate(
                  depth_range_m = case_when(Net.. == 1 ~ "1000-1500", ## Changing net numbers to depth ranges
                                            Net.. == 2 ~ "700-1000",
                                            Net.. == 3 ~ "450-700",
                                            Net.. == 4 ~ "70-450",
                                            Net.. == 5 ~ "0-70")) %>%
...
chord_ccz <- filter(ccz_ltu, Phylum.broader.classification == "Chordata") ### filtering only chordata

filtered_ccz <- filter(chord_ccz, LTU %in% c("Diaphus pacificus",
